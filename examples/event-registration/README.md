...
- **participant.html** – for participants to enter their ID, see how many stores they have visited, and list the store names. If they haven't checked in, the page reminds them to do so.
...
The check-in and store pages include **html5-qrcode** from the CDN:
...
Those pages start the scanner with `{ facingMode: "environment" }` so the back camera is preferred.
